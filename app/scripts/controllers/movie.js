  ...
  .controller('MovieCtrl', function ($scope,$sce,Movies,Settings) {
  	...
  	$scope.title = Settings.get().finnkinoName;
		...
		.then(function(data){
		    ...
		    // group by movie
		    $scope.movies = _.groupBy(jsonData.Schedule.Shows_asArray[0].Show_asArray,function(movie){ return movie.Title });

		    $scope.movies = _.filter($scope.movies, function(movie){ return $scope.filterStartedMovies(movie)});
		...
		$scope.filterStartedShows = function(show){
		   return moment(show.dttmShowStart).toDate() > new Date()
		}

		// filter out already started movies
		$scope.filterStartedMovies = function(movie){
		   return _.filter(movie, function(show){ return $scope.filterStartedShows(show)  }).length
		}
